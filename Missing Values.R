...
aggr_plot <- aggr(test3[,c('PovertyPercent')], 
                  col=c('green','red'), numbers=TRUE, sortVars=TRUE, 
                  labels=c('Poverty Percent'), cex.axis=.7, 
                  gap=3, ylab=c("Histogram of missing data","Pattern"))
# Average Days Between Encounters
# Input max possible value for time interval
#test3$AverageDaysBetweenEncounters<-ifelse(is.na(test3$AverageDaysBetweenEncounters),180,test3$AverageDaysBetweenEncounters)
...
set.seed(1)
misspp<-rfImpute(x = PIG~.,data = test3[,-c(1:2)], iter=1, ntree=10, mtry=ceiling(dim(test3)[2]^(1/3)))
